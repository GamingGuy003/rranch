...
fn main() -> std::io::Result<()> {
    ...
    let args = Vec::from([
        ...
        Arg::new("ll", "latest-log", "Latest job log", None),
        ...
        Arg::new(
            ...
            "Submits release solution",
            Some("path"),
        ),
        Arg::new(
            "ssc",
            "submit-solution-cross",
            "Submits cross solution",
            Some("path"),
        ),
    ...
    let mut client = match Client::new(
        ...
        config.get_master().get_port() as u16,
    ) {
        Ok(client) => client,
        Err(err) => {
    ...
    for parsed in argparser.get_parsed() {
        ...
        let result = match parsed.0.clone().as_str() {
            ...
            "--job-log" => client.show_job_log(parsed.1.unwrap_or_default().as_str()),
            ...
            "--latest-log" => client.show_latest_log(),
            ...
            "--submit-solution-release" => {
                client.submit_solution(parsed.1.unwrap_or_default().as_str(), true)
            }
            "--submit-solution-cross" => {
                client.submit_solution(parsed.1.unwrap_or_default().as_str(), false)
            }
            "--edit" => client.edit(
                parsed.1.unwrap_or_default().as_str(),
                &config.get_client().get_editor(),
            ),
            "--remove-pkg" => client.remove_pkg(parsed.1.unwrap_or_default().as_str()),
            "--extra-sources" => client.show_extra_sources(),
            "--remove-extrasource" => {
                client.remove_extra_source(parsed.1.unwrap_or_default().as_str())
            }
            "--submit-extrasource" => {
                client.submit_extra_source(parsed.1.unwrap_or_default().as_str())
            }
